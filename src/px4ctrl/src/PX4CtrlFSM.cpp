  ...
  : param(param_), controller(controller_) /*, thrust_curve(thrust_curve_)*/
{
  state = MANUAL_CTRL;
  hover_pose.setZero();
  rc_data.mode_ch = param_.mode_ch;
  rc_data.trigger_ch = param_.trigger_ch;
  ROS_INFO("[px4ctrl] Mode channel: %d. Trigger channel: %d", (rc_data.mode_ch + 1), (rc_data.trigger_ch + 1));
}
