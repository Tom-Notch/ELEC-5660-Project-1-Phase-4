...
void RC_Data_t::feed(mavros_msgs::RCInConstPtr pMsg)
{
  ...
  mode = ((double)msg.channels[mode_ch] - 1000.0) / 1000.0;
  gear = ((double)msg.channels[trigger_ch] - 1000.0) / 1000.0;
...
void Battery_Data_t::feed(sensor_msgs::BatteryStateConstPtr pMsg)
{
  ...
  volt = 0.2 * volt + 0.8 * voltage;  // Naive LPF, cell_voltage has a higher frequency
