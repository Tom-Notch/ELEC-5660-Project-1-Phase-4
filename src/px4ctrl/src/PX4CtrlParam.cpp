...
void Parameter_t::config_from_ros_handle(const ros::NodeHandle& nh)
{
  ...
  int mode_tmp = 5, trigger_tmp = 6;
  read_essential_param(nh, "channels/mode", mode_tmp);
  read_essential_param(nh, "channels/trigger", trigger_tmp);
  mode_ch = mode_tmp - 1;
  trigger_ch = trigger_tmp - 1;
